...
def _classical_greedy(villages, shelters, connections, storm_lat, storm_lng, storm_radius_km):
    """
    Classical greedy baseline — intentionally NAIVE.
    It routes people to the chronologically closest shelter (base_time_hrs)
    without regard for the ML-evaluated flood risk or track intersection.
    """
    ...
    for v in sorted(villages, key=lambda x: x["population"], reverse=True):
        # Naive: Sort by physical distance/time ONLY, ignoring the ML storm risk
        routes = sorted(
            [c for c in connections if c["source"] == v["id"]],
            key=lambda c: c["base_time_hrs"]
        )
        assigned = None
        for r in routes:
        ...
        # Classical doesn't use the ML penalty for routing, but we must MEASURE its true risk
        # using the ML data to show why classical fails
        ...
        # True cost if you took this naive route
