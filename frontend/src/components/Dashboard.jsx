...
export default function Dashboard({ region, setRegion, threatData, isOptimizing, optimizedData, stormTrack, onStormParamChange, onRunOptimization }) {
  ...
  return (
    <div className="dashboard-panel">
      ...
      <div style={{
        ...
        <SliderRow label={`Storm Radius (km)`}   param="radius_km"       min={10}  max={200} step={5}  value={stormTrack?.radius_km ?? 30}       fmt={v => `${v} km`} />

        <SliderRow label={`Wind Speed`}           param="wind_speed_kmh"  min={50}  max={220} step={5}  value={stormTrack?.wind_speed_kmh ?? 175}
          fmt={v => { const [cat, col] = windCategory(v); return <><span style={{color: col}}>{cat}</span> · {v} km/h</>; }} />

        <SliderRow label={`Rainfall Intensity`}   param="rainfall_mm"     min={50}  max={400} step={10} value={stormTrack?.rainfall_mm ?? 250}
        ...
        <div style={{ fontSize: '0.68rem', color: '#475569', marginTop: '6px' }}>
          Click the map to reposition storm → hit Optimize to re-run LSTM+QAOA pipeline
        </div>
      </div>
